...
def df_metadata(df, label_col=None, datetime_regex="^\d{4}-\d{2}-\d{2}", time_series_len=None, existing_md=None) -> dict:
    ...
    scaler = sklearn.preprocessing.RobustScaler() # for determining median and interquantile range
    ...
    if label_col in df: # we know which column is the ML label
    ...
    for colpos, colname in enumerate(df):
        ...
        # Deal with NaN's
        md[colname]['nan_percentage'] = df_col.isnull().values.sum() / md['_global']['num_rows']
        df_col = df_col.dropna()

        md[colname]['min'] = df_col.min()
        md[colname]['max'] = df_col.max()
        md[colname]['mode'] = df_col.mode()[0]

        md[colname]['vocab'] = sorted(df_col.unique()) # We'll need this for both numerical and categorical data
        ...
        if pd.api.types.is_numeric_dtype(df_col):
            md[colname]['corr'] = dict(corr[colname]) # Pearson correlation coefficient among all features
            md[colname]['corr'].pop(colname) # we don't need self-correlation
            md[colname]['mean'] = df_col.mean()
            md[colname]['variance'] = df_col.var()
            scaler.fit(pd.DataFrame(df_col))
            md[colname]['median'] = scaler.center_[0]
            md[colname]['iqr'] = scaler.scale_[0] # inter-quantile range. More robust than variance to non-normal data
            md[colname]['skew'] = df_col.value_counts().skew()
            md[colname]['kurtosis'] = df_col.value_counts().kurtosis()
            hist_freqs, hist_ranges = np.histogram(df_col)
            md[colname]['histogram'] = (list(hist_freqs), list(hist_ranges)) # Lists are prolly easier to deal with than np.arrays
        ...
        if 'is_inferred' not in md[colname]  or  md[colname]['is_inferred'] == True: # Don't try to infer if we already told it not to.
            ...
            elif pd.api.types.is_object_dtype(df_col) and re.match(datetime_regex, df_col.iloc[0]):
            ...
            # Maybe some kind of categorical data
                ...
                elif pd.api.types.is_integer_dtype(df_col) and is_ordinal(np.array(df_col)):
                    md[colname]['ds_type'] = 'float'
                    md[colname]['is_inferred'] = True
...
def is_ordinal(arr, hist_bins=12):
    ''' Given an array of ints, infers if the data is ordinal, rather than nominal. '''

    # Basic type check
    if type(arr) != np.ndarray or not isinstance(arr, int):
        ValueError("Error: input to is_ordinal must be a numpy array of ints")
    ...
    hist = np.histogram(arr, bins=hist_bins)
    signs_of_diffs = np.sign(np.diff(hist[0]))
    changes_of_signs = np.sum(np.abs(np.diff(signs_of_diffs)))

    if changes_of_signs < len(hist[0]):
...
def main():
    parser = argparse.ArgumentParser(description='Infer information from a Pandas dataframe')
    ...
    df = pd.read_csv(args.csv, sep=args.sep, na_values='\\N', skipinitialspace=True, header=header_handling)
